...
class TableServiceTest {
    ...
    void getCurrentPlayer() {
        if (tableService.getState().equals(GameState.OPEN)) {
            Assertions.assertThat(tableService.getCurrentPlayer()).isEqualTo(Optional.empty());
        } else {
            Assertions.assertThat(tableService.getCurrentPlayer()).isNotEqualTo(Optional.empty());
        }
    }
    ...
    void getPlayerCardsWhenNotEmpty() {
        String playerId = "003";
        //Mockito.when(playerKailash.getId()).thenReturn(playerId);
        //Mockito.when(playerKailash.getHandCards()).thenReturn(Arrays.asList(card,card));
        tableService.addPlayer(playerId,"Kailash");
        Assertions.assertThat(tableService.getPlayerCards(playerId)).isNotEmpty();
        Assertions.assertThat(tableService.getPlayerCards(playerId).size()).isEqualTo(2);
    }
